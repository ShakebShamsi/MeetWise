   ...
   const handleOpen = (isOpen: boolean) => {
      ...
      if (isOpen) {
         startTransition(async () => {
            try {
               console.log("ShareLinkDialog: Opening, fetching data...");
               ...
               console.log("ShareLinkDialog: Data loaded", { types, quotaStatus, accountConnected });
               ...
               if (!accountConnected) {
                  console.log("ShareLinkDialog: No connected account");
                  return;
               }
               ...
               if (defaultType) {
                  console.log("ShareLinkDialog: Auto-selecting meeting type", defaultType);
                  setSelectedMeetingType(defaultType);
                  const result = await getBookingLinkWithMeetingType(
                     defaultType.slug ?? "",
                  );
                  console.log("ShareLinkDialog: Got booking link", result);
                  setBookingUrl(result.url);
               } else {
                  // No meeting types, show create form
                  console.log("ShareLinkDialog: No meeting types, showing create form");
                  setIsCreatingType(true);
               }
            } catch (error) {
               console.error("ShareLinkDialog: Failed to load meeting types:", error);
               alert("Failed to load meeting types: " + (error instanceof Error ? error.message : "Unknown error"));
            }
         });
      } else {
